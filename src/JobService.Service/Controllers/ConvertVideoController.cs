namespace JobService.Service.Controllers
{
        ...
        ControllerBase
    {
        ...
        [HttpPut("native/{path}")]
        public async Task<IActionResult> FireAndForgetSubmitJobWithNativeSDK(string path, [FromServices] ServiceBusClient client)
        {
            _logger.LogInformation("Sending job: {Path}", path);
            ...
            await using var sender = client.CreateSender("convert-job-queue");

            //SubmitJob<ConvertVideo>
            var job = new ConvertVideo
            {
                Path = path,
                GroupId = groupId,
                Index = 0,
